...
ARTIFACT_REGISTRY_IMAGE_PLATFORM := linux/$(ARTIFACT_REGISTRY_ARCH)
...
artifact-registry.publish: build.init
	...
	@rm -f $(ARTIFACT_REGISTRY_RUNTIME_TARBALL)
	@$(MAKE) -C $(IMAGE_DIR)/$(PROJECT_NAME) img.build.shared \
		PLATFORM=$(ARTIFACT_REGISTRY_PLATFORM) \
		IMAGE_PLATFORMS=$(ARTIFACT_REGISTRY_IMAGE_PLATFORM) \
		IMAGE=$(ARTIFACT_REGISTRY_RUNTIME_IMAGE) \
		BUILD_ARGS="--output=type=docker,dest=$(ARTIFACT_REGISTRY_RUNTIME_TARBALL)"
